...
def get_book_search(search_keyword: Optional[str] = None) -> Dict[str, Any]:
    ...
    url = (
        f"https://api.itbook.store/1.0/search/{search_keyword}"
        if search_keyword
        else "https://api.itbook.store/1.0/new"
    )
    ...
    try:
        ...
        # API 호출 실패 시 에이전트가 중단되지 않도록 빈 결과 반환
        return {"error": str(e), "total": "0", "books": []}
