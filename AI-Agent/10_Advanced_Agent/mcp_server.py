...
# LLM을 활용한 성경 말씀 생성 도구
@mcp.tool()
def daily_quote() -> str:
    """사용자에게 은혜를 주는 성경 말씀을 출력합니다"""
    chat_model = ChatOpenAI(model="gpt-5-mini")
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                "당신은 오늘 하루의 성경 말씀을 알려주는 도우미입니다. 사용자의 말씀 요청이 있을시 성경 구절만 출력합니다.",
            ),
            ("human", "오늘의 말씀을 출력해주세요. "),
        ]
    )
...
마지막으로 daily_quote 을 사용하여 성경 말씀을 출력하고, 따뜻한 말한마디를 덧붙입니다. 
